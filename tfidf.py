...
def tf_idf(sentence):
    ...
    #remove verbs
    verb_tags = ["VBP", "VBN", "VBG","VBD", "VB"]
    tokens = nltk.word_tokenize(sent)
    tags = nltk.pos_tag(tokens)
    for i in range(len(tags)):
        word = [word for word,pos in tags if (pos not in verb_tags)]
    sent= ' '.join(word)
    ...
    AddNewStopWords = ['etc', 'etc.',
                       ...
                       'spanish', 'date', 'year', 'month', 'years', 'months','date month', 'year month', 'also', 'good', 'fluent', 'fair',
                       'mother tongue', 'private', 'limited', 'north', 'south', 'east', 'west', 'day', 'days', 'experience', 'qualifications',
                       'employment', 'history', 'educational', 'academic', 'job', 'be', 'me', 'b,tech', 'm.tech','bachelor of engineering', 'master of technology', 'bachelor of commerce',
                       'master of commerce', 'bachelor of science', 'till', 'until', 'associates', 'inc', 'pvt', 'ltd']
    ...
    sent_lemma = re.sub('\\s+', ' ', sent_lemma)    # remove extra blank spaces 
    keytext=sent_lemma
